...
import type { BrandReport, SmoSummary } from '@/emails/AutopilotReport'
...
const INSTAGRAM_SYSTEM_PROMPT = `Sən Azərbaycan bazarı üçün Instagram məsləhətçisisən. Brend məlumatlarına əsaslanaraq Azerbaycanca qısa, ümumi Instagram dəyərləndirməsi yaz. Heç bir uydurma rəqəm, faiz, və ya statistika yazma. Yalnız brend haqqında bildiklərinə əsaslan.
...
Bu brend haqqında qısa Instagram dəyərləndirməsi yaz. Uydurma rəqəm yazma.`
    ...
    try {
      ...
      if (smoGen?.output_data) {
        ...
        smoSummary = {
          ...
          hashtags: ((smo.hashtags as { main_set?: string[] })?.main_set ?? []).slice(0, 10),
          content_pillars: ((smo.content_pillars as { name: string }[]) ?? []).map(p => p.name),
        }
      }
